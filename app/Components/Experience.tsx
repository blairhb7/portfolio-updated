...
const Experience = () => {
  ...
  return (
    <section className=" ">
        {exp.map((info: {id:number, name:string, title:string, date:string, disc:string, secondline:string }) =>{
          const {id, name, title, disc, date, secondline} = info
          return (
            <div key={id} className=" bg-black hover:text-black text-black flex-col  px-6 lg:px-2 lg:m-10 p-2 z-0  transition duration-700 motion-reduce:transition-none ">
